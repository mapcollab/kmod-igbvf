...
#define DRV_VERSION "2.3.8.2" DRV_DEBUG
			      ...
			      u32 status, u16 vlan)
{
	...
	if (status & E1000_RXD_STAT_VP) {
		...
		/* On some adapters, port VLANs are incorrectly indicated in
		 * the rx descriptor, so only indicate a VLAN tag when we
		 * are actually configured for VLANs.
		 */
		if (test_bit(vid, adapter->active_vlans)) {
#ifndef HAVE_VLAN_RX_REGISTER
			__vlan_hwaccel_put_tag(skb, htons(ETH_P_8021Q), vid);
			/* Fall through to napi_gro_receive */
#else
			vlan_gro_receive(&adapter->rx_ring->napi,
					 adapter->vlgrp, vid, skb);
			goto out;
#endif
		}
	}
...
static int __init igbvf_init_module(void)
{
	...
	printk(KERN_INFO "%s: Copyright (c) 1999-2015 Intel Corporation.\n",
	       igbvf_driver_name);
